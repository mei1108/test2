          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                @click="checkSaleAttrVaue(spuSaleAttrValue,spuSaleAttr.spuSaleAttrValueList)">{{spuSaleAttrValue.saleAttrValueName}}</dd>
              </dl>
          
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="numSal" @change="changeNumSal">
                <a href="javascript:" class="plus" @click="numSal++">+</a>
                <a href="javascript:" class="mins"  @click="numSal>1?numSal--:numSal=1">-</a>
              </div>
              <div class="add">
...
<script>
  ...
  export default {
    ...
    methods:{
      checkSaleAttrVaue(saleAttrVaue,arr){
        arr.forEach(element => {
          element.isChecked=0
        });
        saleAttrVaue.isChecked=1
      },
      changeNumSal(event){
        ...
        if(isNaN(num)||num<1){
          ...
          this.numSal=parseInt(num)
        }
      },
      async addShopCart(){
        try{
          //傳入uuid
          await this.$store.dispatch("updateShopCart",{skuId:this.$route.params.skuid,skuNum:this.numSal})
          sessionStorage.setItem("SKUINFO",JSON.stringify(this.skuInfo))
          this.$router.push({name:"addcartsuccess",query:{skuNum:this.numSal}})
        }catch(err){
          alert(err)
        }
      }
